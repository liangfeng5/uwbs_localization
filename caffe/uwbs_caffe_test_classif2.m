function [ results ] = uwbs_caffe_test_classif2( net_model, net_weigths, mode, files, varargin )
%% Description:
...
% Load params structure with meta data
load(files.gt_meta); 
...
if strcmp('x', mode) || strcmp('x_prob', mode)
    classes_num = length(params.label_val{1} ); %Only for classification
elseif strcmp('y', mode) || strcmp('y_prob', mode)
    classes_num = length(params.label_val{2} ); %Only for classification
elseif strcmp('z', mode) || strcmp('z_prob', mode)
    classes_num = length(params.label_val{3} ); %Only for classification
end
...
if strcmp('x', mode) || strcmp('x_prob', mode)
    ...
    results.label_pred_value = params.label_val{1}( results.label_pred_shift );
    results.label_pred_value = results.label_pred_value(:);
    for samp_i=1:results.samples_num 
        results.gt_value(samp_i) = samples{samp_i}.data.x;
    end
elseif strcmp('y', mode) || strcmp('y_prob', mode)
    results.gt_value = zeros(results.samples_num, 1);
    results.label_pred_value = params.label_val{2}( results.label_pred_shift );
    results.label_pred_value = results.label_pred_value(:);
    for samp_i=1:results.samples_num 
        results.gt_value(samp_i) = samples{samp_i}.data.y;
    end
elseif strcmp('z', mode) || strcmp('z_prob', mode)
    results.gt_value = zeros(results.samples_num, 1);
    results.label_pred_value = params.label_val{3}( results.label_pred_shift );
    results.label_pred_value = results.label_pred_value(:);
    for samp_i=1:results.samples_num 
        results.gt_value(samp_i) = samples{samp_i}.data.z;
    end
end
